...
from coreax.coresets import Coresubset
...
class RPCholeskyState(eqx.Module):
    ...
    :param gramian_diagonal: Cached Gramian diagional.
    """
...
def _greedy_kernel_selection(
    ...
    def _greedy_body(i: int, val: tuple[Array, Array]) -> tuple[Array, ArrayLike]:
        ...
        penalty_update = kernel.compute(data, data[updated_coreset_index])[..., 0]
...
class KernelHerding(RefinementSolver[_Data, HerdingState], ExplicitSizeSolver):
    ...
    Solves the coresubset problem by taking a determinstic, iterative, and greedy
    ...
    :param unique: Boolean that ensures the resulting coresubset will only contain
        unique elements
    :param block_size: Block size parameter passed to :meth:`Kernel.compute_mean`
    :param unroll: Unorll parameter passed to :meth:`Kernel.compute_mean`
    """
    ...
    def refine(
        self, coresubset: Coresubset[_Data], solver_state: HerdingState | None = None
    ) -> tuple[Coresubset[_Data], HerdingState]:
        """
        ...
        def selection_function(i: int, _kernel_similiarity_penalty: ArrayLike) -> Array:
            return jnp.argmax(gramian_row_mean - _kernel_similiarity_penalty / (i + 1))
...
class RandomSample(CoresubsetSolver[_Data, None], ExplicitSizeSolver):
    ...
    :param weighted: If to use dataset weights as selection probabilites
    :param unique: If to sampe without replacement
    """
...
class RPCholesky(CoresubsetSolver[_Data, RPCholeskyState], ExplicitSizeSolver):
    ...
    def reduce(
        ...
        """
        Reduce 'dataset' to a :class:`~Coreax.Coresubset` with 'RPCholesky'.
        ...
        def _greedy_body(
            ...
            """RPCholesky itteration - Algorithm 1 of :cite:`chen2023randomly`."""
...
def _convert_stein_kernel(
    x: ArrayLike, kernel: Kernel, score_matching: ScoreMatching | None
) -> SteinKernel:
    """Convert the kernel to a :class:`SteinKernel`."""
...
class SteinThinning(RefinementSolver[_Data, None], ExplicitSizeSolver):
    ...
    :param kernel: :class:`~coreax.kernel.Kernel` instance implementing a kernel
        function :math:`k: \mathbb{R}^d \times \mathbb{R}^d \rightarrow \mathbb{R}`;
        if 'kernel' is a :class:`SteinKernel` and :code:`score_matching is not None`,
        a new instance of the kernel will be generated where the score function is given
        by :code:`score_matching.match(...)`;
    :param score_matching: Specifies/overwrite the score function of the implied/passed
       :class:`SteinKernel`; if :data:`None`, default to :class:`KernelDensityMatching`
       with sensible defaults unless 'kernel' is a :class:`SteinKernel`, in which case
       the kernel's existing score function is used.
    :param unique: If each index in the resulting coresubset should be unique
    ...
    def refine(
        ...
        # Cannot gaurantee that kernel.base_kernel has a 'length_scale' attribute
