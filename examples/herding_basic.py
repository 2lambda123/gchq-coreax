...
from coreax.solvers.coresubset import KernelHerding as KH
...
def main(out_path: Path | None = None) -> tuple[float, float, float]:
    ...
    # )
    # jax.block_until_ready(herding_object.coreset_indices)
    # print(herding_object.coreset_indices)
    # mmd = MMD(SquaredExponentialKernel(length_scale=length_scale)).compute(data.original_data, herding_object.coreset)
    # print(f"MMD: {mmd}")


    herding_object = KH(herding_key, coreset_size, SquaredExponentialKernel(length_scale=length_scale))
    ho_coreset, _ = herding_object.reduce(Data(data.original_data))
    jax.block_until_ready(ho_coreset)
    print(ho_coreset.unweighted_indices)
    mmd = MMD(SquaredExponentialKernel(length_scale=length_scale)).compute(data.original_data, ho_coreset.coreset)
    print(f"MMD: {mmd}")
